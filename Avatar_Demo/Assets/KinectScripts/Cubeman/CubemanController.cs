...
public class CubemanController : MonoBehaviour 
{
    ...
    private Vector3 initialJointPos;
    private Quaternion initialJointRot;
		...
		for(int i = 0; i < bones.Length; i++) 
		{
			if(bones[i] != null)
			{
				...
				if(manager.IsJointTracked(playerID, joint))
				{
                    ...
                    if(posJoint == initialJointPos && rotJoint == initialJointRot)
                    {
                        changeColorCube(materials[0], i);
                    }
                    else
                    {
                        changeColorCube(materials[1], i);
                    }
				...
				if(bones[i] != null)
				{
					if(bones[i].gameObject.activeSelf)
					{
						...
						if(bones[parI].gameObject.activeSelf)
						{
                            ...
                            if (posJoint == initialPosition)
                            {
                                changeColorLine(materials[0], i);
                            }
                            else
                            {
                                changeColorLine(materials[1], i);
                            }
							bLineDrawn = true;
						}
					}
    ...
    public void changeColorCube(Material mat, int boneIndex)
    {
        bones[boneIndex].gameObject.GetComponent<Renderer>().material = mat;
    }

    public void changeColorLine(Material mat, int lineIndex)
    {
        lines[lineIndex].gameObject.GetComponent<Renderer>().material = mat;
        if(mat == materials[1]) //Se mat de entrada for igual ao mat1 que é usado quando algo mexe então mostra o texto do que mexe
        {
            GlobalVariables.textToShow = textArray[lineIndex]; // conforme o que mexe atribui essa string para a variavel global
        }else
        {
            GlobalVariables.textToShow = "The cake is a Lie"; //Se nada mexe
        }
    }
